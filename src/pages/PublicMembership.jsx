...
export const PublicMembership = () => {
  ...
  const downloadBothCards = async () => {
    ...
    try {
      ...
      linkFront.download = `Membership_Card_Front_${data.membershipId || 'BRG'}.png`;
      linkFront.click();
      ...
      linkBack.download = `Membership_Card_Back_${data.membershipId || 'BRG'}.png`;
      linkBack.click();
    } catch (err) {
  ...
  const downloadSingleCard = async (targetRef, cardType) => {
    if (!targetRef.current || !data || downloading) return;
    setDownloading(true);
    try {
      ...
      link.download = `Membership_Card_${cardType}_${data.membershipId || 'BRG'}.png`;
      link.click();
    } catch (err) {
      console.error('Single card download failed:', err);
      alert(`Could not download ${cardType} card.`);
    } finally {
      setDownloading(false);
    }
  };
