          ...
          <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5">
            ...
            <div className="mt-4 pt-4 border-t border-neutral-100 dark:border-neutral-800">
              <form onSubmit={handleApplyCoupon} className="flex gap-2">
                ...
                {appliedCoupon ? (
                  <button type="button" onClick={() => setAppliedCoupon(null)} className="px-3 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-500 font-semibold text-xs border border-red-200 dark:border-red-500/20 cursor-pointer">Remove</button>
                ) : (
                  <button type="submit" disabled={couponLoading} className="px-4 rounded-xl bg-neutral-900 hover:bg-neutral-800 dark:bg-neutral-800 text-white font-bold text-xs cursor-pointer">{couponLoading ? '...' : 'Apply'}</button>
                )}
              </form>
          ...
          <section className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5">
            ...
            {isAuthenticated ? (
              <div className="flex items-center justify-between p-3 bg-neutral-50 dark:bg-neutral-950 border border-neutral-200/50 dark:border-neutral-800 rounded-xl">
                ...
                <button type="button" onClick={logout} className="flex items-center gap-1 text-[11px] text-red-500 font-bold hover:underline cursor-pointer"><LogOut className="w-3.5 h-3.5" /> Logout</button>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="flex bg-neutral-100 dark:bg-neutral-950 p-1 rounded-xl">
                  <button type="button" onClick={() => { setAuthTab('login'); setAuthError(''); }} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${authTab === 'login' ? 'bg-white dark:bg-neutral-900 text-neutral-800 dark:text-white shadow-sm' : 'text-neutral-500'}`}>Log In</button>
                  <button type="button" onClick={() => { setAuthTab('signup'); setAuthError(''); }} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${authTab === 'signup' ? 'bg-white dark:bg-neutral-900 text-neutral-800 dark:text-white shadow-sm' : 'text-neutral-500'}`}>Register</button>
                </div>
            ...
            <section className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5">
              ...
              <div className="space-y-3.5">
                ...
                <label className="flex items-center gap-2 text-xs font-semibold text-neutral-700 dark:text-neutral-300 cursor-pointer pt-1">
                  <input type="checkbox" checked={billingSame} onChange={(e) => setBillingSame(e.target.checked)} className="w-4 h-4 rounded text-primary-500 focus:ring-primary-500/50 cursor-pointer" /> Billing address same as delivery
                </label>
                {!billingSame && (
          ...
          {isAuthenticated && (
            <section className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => setPaymentMethod('cod')} className={`p-3 rounded-xl border flex flex-col items-center gap-1.5 text-center transition-all cursor-pointer ${paymentMethod === 'cod' ? 'border-primary-500 bg-primary-500/5 text-primary-500' : 'border-neutral-200 dark:border-neutral-800 text-neutral-600 dark:text-neutral-300 hover:border-neutral-300'}`}>
